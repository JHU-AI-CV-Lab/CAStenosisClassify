...
if __name__=="__main__":
    ...
    weight_best_path = os.path.join(H5_PATH, "coronary_weights." + LOG_NAME + ".0_1_2.best." + k_fold_idx + ".hdf5")
    weight_warm_path = os.path.join(H5_PATH, "coronary_weights." + LOG_NAME + ".0_1_2.warm." + k_fold_idx + ".hdf5")
    weight_final_path = os.path.join(H5_PATH, "full_coronary_model." + LOG_NAME + ".0_1_2." + k_fold_idx + ".hdf5")
    num_category = 3
    ...
    angio_df = angio_df[(True ^ angio_df['level'].isin([3]))]
    ...
    train_df = raw_train_df.groupby(['level']).apply(
        lambda x: x.sample(int(raw_train_df[(True ^ raw_train_df['level'].isin([3]))].shape[0] / num_category),
