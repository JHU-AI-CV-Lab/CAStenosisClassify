...
if __name__=="__main__":
    ...
    weight_best_path = os.path.join(H5_PATH, "coronary_weightsC.R.0_2_3_r.best." + k_fold_idx + ".hdf5")
    weight_warm_path = os.path.join(H5_PATH, "coronary_weightsC.R.0_2_3_r.r.warm." + k_fold_idx + ".hdf5")
    weight_final_path = os.path.join(H5_PATH, "full_coronary_modelC.R.0_2_3_r." + k_fold_idx + ".hdf5")
    ...
    angio_df['level_cat'] = angio_df['level'].map(lambda x: to_categorical(x, 1 + angio_df['level'].max()))

    angio_df.dropna(inplace=True)
    angio_df = angio_df[angio_df['exists']]
    print(angio_df.sample(20))
    angio_df[['level']].hist(figsize=(10, 3))
    #show()
    ...
    train_df = raw_train_df.groupby(['level']).apply(lambda x: x.sample(int(raw_train_df.shape[0] / 4), replace=True)
    ...
    tensorboard = TensorBoard(log_dir=os.path.join(LOG_PATH, 'core320', 'InceptionC',
                                                   LOG_NAME, 'R_0-2-3-r_' + k_fold_idx), histogram_freq=0)
    if isReduceLR:
