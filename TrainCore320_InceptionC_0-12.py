...
if __name__=="__main__":
    ...
    weight_best_path = os.path.join(H5_PATH, "coronary_weightsC." + LOG_NAME + ".0_12.best." + k_fold_idx + ".hdf5")
    weight_warm_path = os.path.join(H5_PATH, "coronary_weightsC." + LOG_NAME + ".0_12.warm." + k_fold_idx + ".hdf5")
    weight_final_path = os.path.join(H5_PATH, "full_coronary_modelC." + LOG_NAME + ".0_12." + k_fold_idx + ".hdf5")
    ...
    angio_df = angio_df[(True ^ angio_df['level'].isin([2]))]
    ...
    valid_df = valid_df[(True ^ valid_df['level'].isin([2]))]# Remove redundant frames in validation set
    ...
    train_df = raw_train_df.groupby(['level']).apply(
        lambda x: x.sample(int(raw_train_df[(True ^ raw_train_df['level'].isin([2]))].shape[0] / 2),
                           replace=True)).reset_index(drop=True)
    print('New Data Size:', train_df.shape[0], 'Old Size:', raw_train_df.shape[0])
    train_df[['level']].hist(figsize=(10, num_category))
    #show()
    ...
    #    c_ax.set_title('Severity {}'.format(np.argmax(c_y, -1)))
    #    c_ax.axis('off')
    retina_model = inceptionV3_coronary_model(t_x, t_y, learning_rate=float(lr))
    ...
    tensorboard = TensorBoard(log_dir=os.path.join(LOG_PATH, 'core320', 'InceptionC', LOG_NAME, '0-23-r_' + k_fold_idx), histogram_freq=0)
    if isReduceLR:
    ...
    if useWarm and os.path.exists(weight_warm_path):
        retina_model.load_weights(weight_warm_path)
